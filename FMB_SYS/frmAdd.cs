...
namespace FMB_SYS
{
    public partial class frmAdd : Form
    {
        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            ...
            var labInfo = lab.Barcodes.SingleOrDefault(c => c.MaCode == mabarcode);
            var fmbInfo = fmb.PFmbLabResults.SingleOrDefault(c => c.MaCode == mabarcode);
            if (fmbInfo == null && labInfo != null)
            {
            ...
            else if (fmbInfo != null && labInfo != null)
            {
                ...
                else if (fmbInfo.Place == "FMB Stock")
                {
                    lbError.Text = "Xe: " + fmbInfo.MaNguyenLieu + " này đã ở trong kho\nHàng " + fmbInfo.FmbLine + " vị trí " + fmbInfo.FmbNo + "\nXe nhập kho lúc: " + fmbInfo.InputTime + " bởi: " + fmbInfo.PicInput;
                    lbInformation.Text = "";
                    lbSP.Text = "Thoát hoặc quét mã QR tiếp";
                }
                else if (fmbInfo.Place == null)
                {
                    if (fmbInfo.Kq == "NG" || listArnormal.Visible == true && listArnormal.SelectedItems != null)
                    {
                        ...
                        if (line != null)
                        {
                            var check = fmb.PFmbLabResults.Where(c => c.Place == "FMB Stock").OrderByDescending(c => c.FmbNo).FirstOrDefault(c => c.Labkind == fmbInfo.Labkind);
                            if (check != null)
                            {
